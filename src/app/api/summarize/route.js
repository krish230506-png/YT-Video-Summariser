...
export async function POST(req) {
  try {
    ...
    try {
      ...
      console.log(`Fetching transcript via Spicy-Laika RapidAPI for: ${targetId}`);
      const options = {
        method: 'GET',
        headers: {
      ...
      if (data.message && data.message.includes("subscribed")) {
        return NextResponse.json({ error: 'RapidAPI Error: ' + data.message + ' Please go click "Subscribe to Test" on the RapidAPI page.' }, { status: 403 });
      }
